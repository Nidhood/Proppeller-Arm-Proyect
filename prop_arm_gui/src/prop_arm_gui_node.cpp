...
void PropArmGuiNode::setupPublishers() {
  ...
  sim_ref_angle_cmd_pub_ = this->create_publisher<std_msgs::msg::Float64>(
      "/arm_sim/ref_angle_rad", qos_profile);

  sim_pwm_cmd_pub_ = this->create_publisher<std_msgs::msg::UInt16>(
      "/arm_sim/esc/pwm_us", qos_profile);

  sim_auto_mode_pub_ = this->create_publisher<std_msgs::msg::Bool>(
      "/arm_sim/esc/auto_mode", qos_profile);
}
...
void PropArmGuiNode::armAngleCallback(
  ...
  if (data_recorder_ && data_recorder_->isRecording()) {
    ...
    QMetaObject::invokeMethod(
        ...
        [this, remaining, count]() {
          emit recordingProgress(remaining, count);
        },
        Qt::QueuedConnection);
    ...
    if (!data_recorder_->isRecording() && count > 0) {
      ...
      QMetaObject::invokeMethod(
          this,
          [this, count, duration]() {
            emit recordingCompleted(count, duration);
          },
          Qt::QueuedConnection);
    }
  }
...
void PropArmGuiNode::motorSpeedCallback(
    const std_msgs::msg::Float64::SharedPtr msg) {
  std::lock_guard<std::mutex> lock(data_mutex_);
...
void PropArmGuiNode::pwmInputCallback(
    const std_msgs::msg::UInt16::SharedPtr msg) {
  std::lock_guard<std::mutex> lock(data_mutex_);

  current_data_.pwm_input_us = static_cast<double>(msg->data);
  current_data_.duty_cycle_percent =
      calculateDutyCycle(current_data_.pwm_input_us);
  current_data_.system_timestamp = getSystemTimestamp();
  current_data_.datetime = QDateTime::currentDateTime();
...
void PropArmGuiNode::simArmAngleCallback(
    const std_msgs::msg::Float64::SharedPtr msg) {
  std::lock_guard<std::mutex> lock(data_mutex_);
...
void PropArmGuiNode::simMotorSpeedCallback(
    const std_msgs::msg::Float64::SharedPtr msg) {
  std::lock_guard<std::mutex> lock(data_mutex_);
...
void PropArmGuiNode::simPwmInputCallback(
    const std_msgs::msg::UInt16::SharedPtr msg) {
  std::lock_guard<std::mutex> lock(data_mutex_);
  current_data_.sim_pwm_input_us = static_cast<double>(msg->data);
  current_data_.sim_duty_cycle_percent =
      calculateDutyCycle(current_data_.sim_pwm_input_us);
  QMetaObject::invokeMethod(
      this, [this]() { emit dataUpdated(); }, Qt::QueuedConnection);
}

void PropArmGuiNode::simRefAngleCallback(
    const std_msgs::msg::Float64::SharedPtr msg) {
  std::lock_guard<std::mutex> lock(data_mutex_);
...
std::vector<PropArmData>
PropArmGuiNode::getHistoryData(size_t max_points) const {
  std::lock_guard<std::mutex> lock(data_mutex_);
...
void PropArmGuiNode::sendAngleCommand(double angle_rad) {
  ...
  if (sim_ref_angle_cmd_pub_) {
    sim_ref_angle_cmd_pub_->publish(msg);
  }
...
void PropArmGuiNode::sendAutoModeCommand(bool auto_mode) {
  ...
  if (sim_auto_mode_pub_) {
    sim_auto_mode_pub_->publish(msg);
  }
...
void PropArmGuiNode::sendStopCommand() {
  ...
  if (sim_ref_angle_cmd_pub_) {
    sim_ref_angle_cmd_pub_->publish(angle_msg);
  }
  ...
  if (sim_pwm_cmd_pub_) {
    sim_pwm_cmd_pub_->publish(pwm_msg);
  }
...
void PropArmGuiNode::startStepTest(double angle_low_deg, double angle_high_deg,
                                   double time_up_s, double time_down_s) {
...
void PropArmGuiNode::clearRecordedData() {
  if (data_recorder_) {
    data_recorder_->clearRecording();
  }
}
...
void PropArmGuiNode::stepTestTimerCallback() {
  ...
  double phase_duration =
      step_high_phase_ ? step_time_up_s_ : step_time_down_s_;
  ...
  if (elapsed >= phase_duration) {
    ...
    double target_angle =
        step_high_phase_ ? step_angle_high_deg_ : step_angle_low_deg_;
    sendAngleCommand(target_angle * M_PI / 180.0);
  }
}

bool PropArmGuiNode::isConnected() const { return connected_; }

std::string PropArmGuiNode::getConnectionStatus() const {
  return connected_ ? "Connected" : "Disconnected";
}

std::string PropArmGuiNode::getControlMode() const { return control_mode_; }
...
void PropArmGuiNode::startRecording(double duration_seconds) {
  if (data_recorder_) {
    ...
    QMetaObject::invokeMethod(
        this,
        [this, duration_seconds]() { emit recordingStarted(duration_seconds); },
        Qt::QueuedConnection);
  }
}
...
void PropArmGuiNode::stopRecording() {
  if (data_recorder_) {
    ...
    QMetaObject::invokeMethod(
        this,
        [this, count, duration]() { emit recordingCompleted(count, duration); },
        Qt::QueuedConnection);
  }
}
