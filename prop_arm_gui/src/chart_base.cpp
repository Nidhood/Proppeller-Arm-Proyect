...
HoverChartView::HoverChartView(QWidget *parent)
    : QChartView(parent), hover_enabled_(true), mouse_in_plot_area_(false),
      hover_line_(nullptr), hover_point_(nullptr), hover_text_(nullptr) {
  setMouseTracking(true);
  setRenderHint(QPainter::Antialiasing);
}

HoverChartView::~HoverChartView() { clearHoverElements(); }
...
ChartBase::ChartBase(const ChartConfig &config, QWidget *parent)
    : QWidget(parent), config_(config),
      ...
      chart_(nullptr), chart_view_(nullptr), main_series_(nullptr),
      trend_series_(nullptr), smooth_series_(nullptr),
      sim_main_series_(nullptr), sim_smooth_series_(nullptr), x_axis_(nullptr),
      y_axis_(nullptr), data_mutex_(new QMutex()), pool_write_index_(0),
      optimal_buffer_size_(1000), update_timer_(nullptr), last_update_time_(0),
      last_hover_update_time_(0), update_pending_(false),
      hover_update_pending_(false), data_initialized_(false),
      hover_active_(false), hover_persistent_(false),
      hover_fixed_time_absolute_(-1.0), cached_hover_value_(0.0),
...
void ChartBase::createLayout() {
  ...
  connect(chart_view_, &HoverChartView::hoverUpdate, this,
          &ChartBase::onHoverUpdate);
}
...
void ChartBase::updateAxes() {
  ...
  if (!data_points_.empty()) {
    current_time = data_points_.back().timestamp;
  } else if (!sim_data_points_.empty()) {
    current_time = sim_data_points_.back().timestamp;
  } else {
  ...
  if (config_.auto_scale && !data_points_.empty()) {
    auto [min_it, max_it] =
        std::minmax_element(data_points_.begin(), data_points_.end(),
...
void ChartBase::initializeMemoryPool() {
  optimal_buffer_size_ =
      std::min(static_cast<size_t>(2000), config_.max_points * 2);
...
void ChartBase::addSimDataPoint(double value, double timestamp) {
  ...
  while (!sim_data_points_.empty() &&
         sim_data_points_.front().timestamp < cutoff_time) {
...
std::vector<QPointF> ChartBase::calculateSplinePoints(
    const std::vector<QPointF> &control_points) const {
  ...
  for (size_t i = 0; i < control_points.size() - 1; ++i) {
    ...
    const QPointF &p3 = (i < control_points.size() - 2) ? control_points[i + 2]
                                                        : control_points[i + 1];
...
void ChartBase::onHoverUpdate(QPointF chart_position, QPoint scene_position,
                              bool valid) {
...
void ChartBase::updateHoverDisplay() {
  ...
  QPointF chart_at_x =
      sceneToChartCoords(QPoint(hover_fixed_scene_x_, current_scene_pos_.y()));
...
void ChartBase::createHoverElements(QGraphicsScene *scene, const QPointF &,
                                    const QPoint &scene_pos, double value,
                                    double time) {
  ...
  chart_view_->hover_line_ = scene->addLine(
      scene_pos.x(), chart_->plotArea().top(), scene_pos.x(),
      chart_->plotArea().bottom(), QPen(TEXT_SECONDARY, 1, Qt::DashLine));

  chart_view_->hover_point_ = scene->addEllipse(
      scene_pos.x() - 4, scene_pos.y() - 4, 8, 8,
      QPen(config_.primary_color, 2), QBrush(config_.primary_color));
...
QPointF ChartBase::findNearestPointOnCurve(double absolute_target_time) const {
  ...
  auto it = std::lower_bound(data_points_.begin(), data_points_.end(),
                             absolute_target_time,
  ...
  if (it == data_points_.end()) {
    return QPointF(data_points_.back().timestamp - start_time_,
                   data_points_.back().value);
  }
...
double ChartBase::interpolateValue(double target_time) const {
  ...
  auto it =
      std::lower_bound(data_points_.begin(), data_points_.end(), target_time,
...
QPoint ChartBase::chartToSceneCoords(const QPointF &chart_pos) const {
  ...
  return QPoint(static_cast<int>(scene_pos.x()),
                static_cast<int>(scene_pos.y()));
}
...
void ChartBase::setYRange(double min, double max) {
  ...
  if (!config_.auto_scale) {
    QMetaObject::invokeMethod(
        this, [this, min, max]() { y_axis_->setRange(min, max); },
        Qt::QueuedConnection);
  }
}
...
void ChartBase::setAutoScale(bool enabled) {
  ...
  if (!enabled) {
    QMetaObject::invokeMethod(
        this, [this]() { y_axis_->setRange(config_.y_min, config_.y_max); },
        Qt::QueuedConnection);
  }
}
...
void ChartBase::setGridVisible(bool visible) {
  ...
  QMetaObject::invokeMethod(
      ...
      [this, visible]() {
        ...
        y_axis_->setMinorGridLineVisible(visible && config_.show_minor_grid);
      },
      Qt::QueuedConnection);
}
...
void ChartBase::setSmoothCurves(bool enabled) {
  ...
  QMetaObject::invokeMethod(
      ...
      [this]() {
        ...
        if (config_.show_sim_series && !current_sim_data.isEmpty()) {
          if (config_.use_smooth_curves && sim_smooth_series_) {
...
void ChartBase::updateChart() { update_pending_ = true; }

void ChartBase::updateTheme() {
  QMetaObject::invokeMethod(
      this, [this]() { applyProfessionalTheme(); }, Qt::QueuedConnection);
}

std::vector<double>
ChartBase::calculateSplineCoefficients(const std::vector<double> &x,
